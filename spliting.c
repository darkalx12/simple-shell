...
/**
 * spliting_commands - splits command lines according to
...
int spliting_commands(data_shell *datash, char *input)
{
	...
	while (list_l != NULL)
	{
		...
		datash->args = spliting_line(datash->input);
...
/**
 * spliting_line - tokenizes the input string
 *
 * @input: input string.
...
char **spliting_line(char *input)
{
	...
	for (i = 1; token != NULL; i++)
	{
		...
		{
			bsize += TOK_BUFSIZE;
			tokens = _realcdp(tokens, i, sizeof(char *) * bsize);
			if (tokens == NULL)
			{
				write(STDERR_FILENO, ": allocation error\n", 18);
				exit(EXIT_FAILURE);
